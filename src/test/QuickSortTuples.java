...
//      TestSort.printArray(data);
  ...
  private static int partition(String[][] data, int first, int last) {
    ...
    String[] pivot = data[mid];        //    so in-order not worst case
    ...
    while (Integer.valueOf(data[iHi][0]) > Integer.valueOf(pivot[0])) // normal downward scan
      iHi--;
    while (iLo <= iHi && Integer.valueOf(data[iLo][0]) < Integer.valueOf(pivot[0])) // no sentinel upward yet
        iLo++;
    if (iLo <= iHi) {
      String[] temp = data[iLo];
      data[iLo] = data[iHi];
      data[iHi] = temp;
      iHi--;
      iLo++;
    ...
    while (iLo <= iHi) { // now have sentinels both ways
      while (Integer.valueOf(data[iHi][0]) > Integer.valueOf(pivot[0])) 
        iHi--;
      while (Integer.valueOf(data[iLo][0]) < Integer.valueOf(pivot[0])) // have sentenel swapped in place now
        iLo++;
      if (iLo <= iHi) {
        String[] temp = data[iLo];
        data[iLo] = data[iHi];
  ...
  public static void insertionsort(String[][] data, int start, int end) {
    for (int next = start + 1; next <= end; next++) {
      if (Integer.valueOf(data[next-1][0]) > Integer.valueOf(data[next][0])) {
        String[] val = data[next];
        int gap = next-1;
        data[next] = data[gap];
        while (gap > start && Integer.valueOf(data[gap-1][0]) > Integer.valueOf(val[0])) {
          data[gap] = data[gap-1];
          gap--;
        }
        data[gap] = val;
