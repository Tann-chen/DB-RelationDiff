...
*/
public class QuickSortTuplesObject {
   ...
   */
  public static void quicksort(Tuple[] data, int start, int end) {
...
//      TestSort.printArray(data);
  ...
  private static int partition(Tuple[] data, int first, int last) {
    ...
    Tuple pivot = data[mid];        //    so in-order not worst case
    ...
    while (data[iHi].id > pivot.id) // normal downward scan
      iHi--;
    while (iLo <= iHi && data[iLo].id < pivot.id) // no sentinel upward yet
        iLo++;
    if (iLo <= iHi) {
      Tuple temp = data[iLo];
      data[iLo] = data[iHi];
    ...
    while (iLo <= iHi) { // now have sentinels both ways
      while (data[iHi].id > pivot.id) 
        iHi--;
      while (data[iLo].id < pivot.id) // have sentenel swapped in place now
        iLo++;
      if (iLo <= iHi) {
        Tuple temp = data[iLo];
        data[iLo] = data[iHi];
  ...
  public static void insertionsort(Tuple[] data, int start, int end) {
    for (int next = start + 1; next <= end; next++) {
      if (data[next-1].id > data[next].id) {
        Tuple val = data[next];
        int gap = next-1;
        data[next] = data[gap];
        while (gap > start && data[gap-1].id > val.id) {
          data[gap] = data[gap-1];
          gap--;
        }
        data[gap] = val;
