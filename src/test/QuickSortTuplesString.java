...
*/
public class QuickSortTuplesString {
...
//      TestSort.printArray(data);
  ...
  private static int partition(String[][] data, int first, int last) {
    ...
    while (data[iHi][0].compareTo(pivot[0]) > 0) // normal downward scan
      iHi--;
    while (iLo <= iHi && data[iLo][0].compareTo(pivot[0]) < 0) // no sentinel upward yet
        iLo++;
    if (iLo <= iHi) {
    ...
    while (iLo <= iHi) { // now have sentinels both ways
      while (data[iHi][0].compareTo(pivot[0]) > 0) 
        iHi--;
      while (data[iLo][0].compareTo(pivot[0]) < 0) // have sentenel swapped in place now
        iLo++;
      if (iLo <= iHi) {
        String[] temp = data[iLo];
        data[iLo] = data[iHi];
  ...
  public static void insertionsort(String[][] data, int start, int end) {
    for (int next = start + 1; next <= end; next++) {
      if (data[next-1][0].compareTo(data[next][0]) > 0) {
        String[] val = data[next];
        int gap = next-1;
        data[next] = data[gap];
        while (gap > start && data[gap-1][0].compareTo(val[0]) > 0) {
          data[gap] = data[gap-1];
          gap--;
        }
        data[gap] = val;
